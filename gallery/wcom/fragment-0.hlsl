...
vec3 wcom(vec3 col, vec2 p) {
  ...
  vec2 n = mod2(p, unit2*(1.5*sc));
  const float bw = 0.1;
  float dh = box(p, unit2*((0.5-bw)*sc))-bw*sc;
  dh = max(dh, box(op, sc*vec2(1.25, 2.75)));

  vec3 gcol0 = hsv2rgb(vec3(0.6, 0.85, 0.0025));

  float v = mod(n.x+n.y, 2.0) == 0.0 ? 0. : 2.0;
  vec3 bcol = hsv2rgb(vec3(0.62, 0.925, v));
  col = mix(col, bcol, 0.3*smoothstep(aa, -aa, dh));
